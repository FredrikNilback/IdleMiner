...
public class AluminiumMine {
    ...
    public AluminiumMine(Player player, int cycle, int resource_amount, int level_scaling, int start_off_investment) {
        this.player = player;
        this.cycle = cycle;
    ...
    public AluminiumMine(Player player, int cycle, int resource_amount, int level_scaling, int start_off_investment, int level, int invested_money, int speed) {
        this.player = player;
        this.cycle = cycle;
        this.resource_amount = resource_amount;
        this.level_scaling = level_scaling;
    ...
    public void mine() {
        timer = new Timer();
        timer.scheduleAtFixedRate(new LeadMiningTask(), 0, (this.cycle * 100000 / speed));
    }

    private class LeadMiningTask extends TimerTask {
        @Override
        public void run() {
            int produced = produceResources();
            System.out.println("Lead mine produced " + produced + " resources.");
            player.updateRawResource(5, produced);
        }
    }
    ...
    private void checkLevelUp() {
        ...
        if(level_up_condition <= this.invested_money) {
            ...
            System.out.println("Aluminium mine level up! New level: " + this.level + "\nCurrent gold invested: " + this.invested_money);
            checkLevelUp();
        }  
    }
