...
public class LithiumMine {
    ...
    public LithiumMine(Player player, int cycle, int resource_amount, int level_scaling, int start_off_investment) {
        this.player = player;
        this.cycle = cycle;
    ...
    public LithiumMine(Player player, int cycle, int resource_amount, int level_scaling, int start_off_investment, int level, int invested_money, int speed) {
        this.player = player;
        this.cycle = cycle;
        this.resource_amount = resource_amount;
        this.level_scaling = level_scaling;
    ...
    private class LeadMiningTask extends TimerTask {
        ...
        public void run() {
            ...
            player.updateRawResource(11, produced);
        }
    }
    ...
    private void checkLevelUp() {
        ...
        if(level_up_condition <= this.invested_money) {
            ...
            System.out.println("Lithium mine level up! New level: " + this.level + "\nCurrent gold invested: " + this.invested_money);
            checkLevelUp();
        }  
    }
